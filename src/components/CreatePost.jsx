...
const CreatePost = ({ open, setOpen }) => {
  ...
  const createPostHandler = async (e) => {
    const formData = new FormData();
    formData.append("caption", caption);
    formData.append("rating", rating); // ⭐ Sending rating to the backend
    if (imagePreview) formData.append("image", file);
  ...
  return (
    <Dialog open={open}>
      <DialogContent onInteractOutside={() => setOpen(false)}>
        ...
        {/* Select Image Button */}
        <input ref={imageRef} type='file' className='hidden' onChange={fileChangeHandler} />
        <Button onClick={() => imageRef.current.click()} className='w-fit mx-auto bg-[#d32f2f] hover:bg-[#b71c1c] text-white'>
        ...
        {imagePreview && (
          loading ? (
            ...
            <Button onClick={createPostHandler} type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white">
              Post
            </Button>
          )
        )}
